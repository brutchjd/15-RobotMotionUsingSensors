...
class SimpleRoseBot(object):
    ...
    def go_straight_for_inches(self, inches, speed):
        start_pos = self.left_wheel_motor.get_position()

        self.left_wheel_motor.turn_on(speed)
        self.right_wheel_motor.turn_on(speed)

        while True:
            pos = self.left_wheel_motor.get_position()
            if ((pos - start_pos)/360) * (1.3 * math.pi) >= inches:
                break
    ...
    def go_straight_until_black(self, speed):
        ...
        while True:
            intensity = self.color_sensor.get_reflected_light_intensity()
            if intensity < 40:
                break
